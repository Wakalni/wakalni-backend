...
export class RestaurantService {
  ...
  async create(createRestaurantDto: CreateRestaurantDto): Promise<Restaurant> {
    ...
    if (existingRestaurant) {
      throw new ConflictException(
        'Restaurant with this name already exists in this city',
      );
    }
    ...
    if (createRestaurantDto.opening_hours) {
      restaurant.opening_hours = createRestaurantDto.opening_hours.map(
        (hoursDto) => this.openingHoursRepository.create(hoursDto),
      );
    }
  ...
  async update(
    ...
    if (updateRestaurantDto.opening_hours) {
      ...
      restaurant.opening_hours = updateRestaurantDto.opening_hours.map(
        (hoursDto) => this.openingHoursRepository.create(hoursDto),
      );
    }
